...
void test()
{
	printf("add a line for test in client1\n");
}
...
void
qla2x00_async_iocb_timeout(void *ptr)
{
	srb_t *sp = ptr;
	...
	case SRB_LOGIN_CMD:
		...
		qla24xx_handle_plogi_done_event(sp->vha, &ea);
		break;
...
qla2x00_async_logout(struct scsi_qla_host *vha, fc_port_t *fcport)
{
	...
	ql_dbg(ql_dbg_disc, vha, 0x2070,
		"Async-logout - hdl=%x loopid=%x portid=%06x %8phC.\n",
...
static void
qla24xx_async_gnl_sp_done(void *ptr, int res)
{
	struct srb *sp = ptr;
	struct scsi_qla_host *vha = sp->vha;
	unsigned long flags;
...
void qla24xx_async_gpdb_sp_done(void *ptr, int res)
{
	struct srb *sp = ptr;
	struct scsi_qla_host *vha = sp->vha;
	struct qla_hw_data *ha = vha->hw;
...
static void
qla2x00_tmf_iocb_timeout(void *ptr)
{
	srb_t *sp = ptr;
...
static void
qla24xx_abort_iocb_timeout(void *ptr)
{
	srb_t *sp = ptr;
...
qla2x00_initialize_adapter(scsi_qla_host_t *vha)
{
	...
	if (ha->flags.disable_serdes) {
		...
		ql_log(ql_log_info, vha, 0x0077,
		    "Masking HBA WWPN %016llx (via NVRAM).\n",
...
void qla2xxx_print_sfp_info(struct scsi_qla_host *vha)
{
#define STR_LEN 64
...
qla24xx_config_rings(struct scsi_qla_host *vha)
{
	...
	if (ha->flags.iocb_fabric_priority)
		icb->firmware_options_3 |= cpu_to_le32(BIT_28);
...
qla2x00_alloc_fcport(scsi_qla_host_t *vha, gfp_t flags)
{
	...
	fcport->ct_desc.ct_sns = dma_alloc_coherent(&vha->hw->pdev->dev,
	    sizeof(struct ct_sns_pkt), &fcport->ct_desc.ct_sns_dma, GFP_ATOMIC);
...
qla2x00_configure_loop(scsi_qla_host_t *vha)
{
	...
	} else if (!vha->flags.online || test_bit(ABORT_ISP_ACTIVE, &flags)) {
		set_bit(RSCN_UPDATE, &flags);
		set_bit(LOCAL_LOOP_UPDATE, &flags);
	}
	...
	if (!rval && test_bit(RSCN_UPDATE, &flags)) {
	...
	if (!rval) {
		if (atomic_read(&vha->loop_down_timer) ||
			...
			ql_dbg(ql_dbg_disc, vha, 0x2069, "LOOP READY.\n");
		}
	}
...
qla2x00_iidma_fcport(scsi_qla_host_t *vha, fc_port_t *fcport)
{
	...
	if (rval != QLA_SUCCESS) {
		ql_dbg(ql_dbg_disc, vha, 0x2004,
		    ...
		    "%016llx -- %04x %x %04x %04x.\n",
		    wwn_to_u64(fcport->port_name), rval,
		    fcport->fp_speed, mb[0], mb[1]);
	} else {
		ql_dbg(ql_dbg_disc, vha, 0x2005,
		    "iIDMA adjusted to %s GB/s on %016llx.\n",
...
qla2x00_configure_fabric(scsi_qla_host_t *vha)
{
	...
	loop_id = IS_FWI2_CAPABLE(ha) ? NPH_F_PORT : SNS_FL_PORT;
	rval = qla2x00_get_port_name(vha, loop_id, vha->fabric_node_name, 1);
	if (rval) {
		ql_dbg(ql_dbg_disc, vha, 0x201f,
		    "Failed to get Fabric Node Name, No FL Port.\n");
	...
	rval = ha->isp_ops->fabric_login(vha, loop_id,
	    0xff, 0xff, 0xfc, mb, BIT_1|BIT_0);
	if (rval || mb[0] != MBS_COMMAND_COMPLETE) {
		ql_dbg(ql_dbg_disc, vha, 0x2042,
		    "Failed SNS login: loop_id=%x "
	...
	if (ql2xfdmienable) {
		if (test_and_clear_bit(REGISTER_FDMI_NEEDED, &vha->dpc_flags)) {
			qla2x00_fdmi_register(vha);
		}
	}

	if (test_and_clear_bit(REGISTER_FC4_NEEDED, &vha->dpc_flags)) {
		if (qla2x00_rft_id(vha) || qla2x00_rff_id(vha) ||
		    qla2x00_rnn_id(vha) || qla2x00_rsnn_nn(vha)) {
			ql_dbg(ql_dbg_disc, vha, 0x2045,
			    "Register FC-4 operation failed.\n");
			if (test_bit(LOOP_RESYNC_NEEDED, &vha->dpc_flags))
				goto done;
		}
	}
...
qla81xx_nvram_config(scsi_qla_host_t *vha)
{
	...
	if (IS_FABRIC_PRIORITY_CAPABLE(ha)) {
		ha->flags.iocb_fabric_priority =
		    (nv->port_flags & NPF_FABRIC_PRIORITY) || ql2xfabric_priority;
		ql_dbg(ql_dbg_init, vha, 0x70e4, "Fabric Priority: %s.\n",
		    ha->flags.iocb_fabric_priority ? "enabled" : "disabled");
	}
