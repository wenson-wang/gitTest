...
void test()
{
	printf("add a line for test in client2\n");
}
static int
...
mbx_done:
	if (rval) {
		if (ql2xextended_error_logging & (ql_dbg_disc|ql_dbg_mbx)) {
			pr_warning("%s [%s]-%04x:%ld: **** Failed", QL_MSGHDR,
			    dev_name(&ha->pdev->dev), 0x1020+0x800, vha->host_no);
			mboxes = mcp->in_mb;
			cnt = 4;
...
static inline
bool qla27xx_is_open_power(struct scsi_qla_host *vha)
{
	mm_segment_t oldfs;
	struct file *fp = NULL;
	char *buf;
	...
	buf = vmalloc(size);
	if (!buf) {
		ql_dbg(ql_dbg_mbx, vha, 0x11a1, "buffer alloc failed\n");
		goto exit;
	}

	fp = filp_open("/proc/cpuinfo", O_RDONLY, 0);
	if (!fp) {
		ql_dbg(ql_dbg_mbx, vha, 0x11a2, "file open failed\n");
		goto exit;
	}

	oldfs = get_fs();
	set_fs(get_ds());
	len = fp->f_op->read(fp, buf, size, &fp->f_pos);
	set_fs(oldfs);

	if (len < 0) {
		ql_dbg(ql_dbg_mbx, vha, 0x11a3, "file read failed (%x)\n", len);
		goto exit;
	}

	buf[len] = 0;
	str = strstr(buf, "POWER8");
exit:
	ql_dbg(ql_dbg_mbx, vha, 0x11a0, "cpuarch=%.6s\n", str);
...
static inline
uint16_t qla25xx_set_sfp_lr_dist(struct qla_hw_data *ha)
{
	uint16_t mb4 = BIT_0;
...
static inline
uint16_t qla25xx_set_nvr_lr_dist(struct qla_hw_data *ha)
{
	uint16_t mb4 = BIT_0;
	...
	if (rval != QLA_SUCCESS) {
		...
		if (IS_FWI2_CAPABLE(ha)) {
			...
			ql_dbg(ql_dbg_mbx, vha, 0x1027,
			    "exchanges=%x.\n", mcp->mb[1]);
			if (IS_QLA27XX(ha)) {
				ha->max_supported_speed = mcp->mb[2] & BIT_0;
				ql_dbg(ql_dbg_mbx, vha, 0x119b,
				    "max_supported_speed=%s.\n",
				    ha->max_supported_speed ? "32Gps" : "16Gps");
...
qla2x00_init_firmware(scsi_qla_host_t *vha, uint16_t size)
{
	...
	if (rval != QLA_SUCCESS) {
		...
		if ((IS_QLA27XX(ha) && (mcp->mb[2] == 6 || mcp->mb[3] == 2)))
		...
		if (IS_QLA83XX(ha) || IS_QLA27XX(ha)) {
			if (mcp->mb[2] == 6 || mcp->mb[3] == 2)
				ql_dbg(ql_dbg_mbx, vha, 0x119e,
				    "Invalid SFP/Validation Failed\n");
			if (mcp->mb[3] & BIT_15)
				ql_log(ql_log_info, vha, 0x11a1,
				    "FEC=enabled (fw state).\n");
		}
